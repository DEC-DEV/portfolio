    ...
    <script>
		$(function () { // 페이지가 로드되면
			...
			$('#btnEdit').click(function () { /*등록 버튼 클릭시 작동, 주의사항 : input type = "button"*/
				/* alert("btnEdit"); */
				document.form1.action="${path}/counsel_update_password_check.do?${_csrf.parameterName}=${_csrf.token}"
		...
		function comment_add() {
			...
			$.ajax({
				type: "POST",
				url: "${path}/counsel_coment_add.do?${_csrf.parameterName}=${_csrf.token}",
				data: param,
		...
		function comment_list() {
			$.ajax({
				type: "POST",
				url: "${path}/counsel_coment_list_search.do?${_csrf.parameterName}=${_csrf.token}",
				data: "num=${dto.b_num}",
	                            ...
	                            <table class="table table-hover">
								  <thead>
								    <tr>
									    ...
									    <td class="col-md-2">${dto.b_date}</td>
								    </tr>
								    <tr>
								  ...
								  <tbody>
								    <tr>
								    	<td colspan="5"><textarea cols="125" rows="10" readonly style="border:none">${dto.b_content}</textarea></td>
								    </tr>
								    <tr>
