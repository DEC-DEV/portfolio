                ...
                <div class="row justify-content-center">
                    <div class="col-lg-7" style="padding-top:80">
                        ...
                        <div class="wow fadeInUp" data-wow-delay="0.3s">
							  ...
							  <tbody>
								  ...
								  <c:forEach var="dto" items="${list}">
								  	  <tr>
									 	  ...
									 	  <td>
										 	  <a href="${path}/counsel_detail_search.do?num=${dto.b_num}&${_csrf.parameterName}=${_csrf.token}">${dto.b_title}</a>
										 	  &nbsp;
										...
										<td colspan="4" align="center">
											<div class="links">
												...
												<c:if test="${paging.startPage > 10}">
													<a href="${path}/counsel_list_search.do?pageNum=${paging.prev}&${_csrf.parameterName}=${_csrf.token}">[이전]</a>
												</c:if>
												<!-- 페이징 번호처리 -->
												<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
													<a href="${path}/counsel_list_search.do?pageNum=${num}&${_csrf.parameterName}=${_csrf.token}">${num}</a>
												</c:forEach>
												<!-- 다음버튼 활성화 여부 -->
												<c:if test="${paging.endPage < paging.pageCount}">
													<a href="${path}/counsel_list_search.do?pageNum=${paging.next}&${_csrf.parameterName}=${_csrf.token}">[다음]</a>
												</c:if>
											</div>
										</td>
									</tr>
							...
							<div>
								<button class="btn btn-primary w-10 py-3" type="button" onclick="window.location='counsel_insert.do?${_csrf.parameterName}=${_csrf.token}'">글쓰기</button>
							</div>
                        </div>
                    </div>
                </div>
