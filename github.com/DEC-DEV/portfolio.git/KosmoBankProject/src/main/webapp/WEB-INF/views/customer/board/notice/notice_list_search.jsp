							  ...
							  <tbody>
								  <c:forEach var="dto" items="${list}">
								  	  <tr>
									 	  ...
									 	  <td>
										 	  <a href="${path}/notice_detail_search.do?num=${dto.n_board_num}&${_csrf.parameterName}=${_csrf.token}">${dto.n_title}</a>
									 	  </td>
									 	  <td>${dto.n_writer}</td>
									 	  <td>${dto.n_date}</td>
									 	  <td>${dto.n_views}</td>
										...
										<td colspan="5" align="center">
											<div class="links">
												...
												<c:if test="${paging.startPage > 10}">
													<a href="${path}/notice_list_search.do?pageNum=${paging.prev}&${_csrf.parameterName}=${_csrf.token}">[이전]</a>
												</c:if>
												<!-- 페이징 번호처리 -->
												<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
													<a href="${path}/notice_list_search.do?pageNum=${num}&${_csrf.parameterName}=${_csrf.token}">${num}</a>
												</c:forEach>
												<!-- 다음버튼 활성화 여부 -->
												<c:if test="${paging.endPage < paging.pageCount}">
													<a href="${path}/notice_list_search.do?pageNum=${paging.next}&${_csrf.parameterName}=${_csrf.token}">[다음]</a>
												</c:if>
											</div>
										</td>
									</tr>
