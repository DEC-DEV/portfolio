                ...
                <div class="row justify-content-center">
                    <div class="col-lg-7" style="padding-top:80">
                        <div class="section-title position-relative text-center mb-5 pb-2 wow fadeInUp" data-wow-delay="0.1s">
                            ...
                            <h2 class="mt-2">대출해지현황조회</h2>
                        </div>
                        <div class="wow fadeInUp" data-wow-delay="0.3s">
							<table class="table table-hover">
								<thead style="background-color: #90acfc">
									<tr>
								...
								<tbody>
									<c:forEach items="${list}" var="item">
										<tr>
											<td>${item.d_num}</td>
											<td>${item.d_name}</td>
											<td><fmt:formatNumber value="${item.d_amount}" pattern="#,###" /></td>
											<td>${item.account_num}</td>
											<c:if test="${item.d_state == 2}">
												<td>해지</td>
											</c:if>
											<td>${item.d_start_date}</td>
											<td>${item.d_end_date}</td>
											<td>${item.d_month}개월</td>
											<td>${item.d_repay}</td>
											<td>${item.d_rate}%</td>
											<td>${item.d_prepayment_fee}%</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
