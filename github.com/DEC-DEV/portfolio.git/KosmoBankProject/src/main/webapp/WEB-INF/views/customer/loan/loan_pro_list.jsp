    ...
    <div id="wrapper">
     ...
     <div class="container-fluid align-bottom mt-5 pl-5 pr-5" style="padding-top: 4em; width:70%" >
       <div class="section-title position-relative text-center mb-5 pb-2 wow fadeInUp" data-wow-delay="0.1s">
              ...
              <h2 class="mt-2">대출 상품 리스트</h2>
          </div>
     	<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
	    <ul class="card list-group list-group-flush">
	    <c:forEach items="${list}" var="item">
		  <li class="list-group-item" style="padding:25px; height:16.5em;">  
		  	<h4 class="mb-1" style="color:#008485">${item.d_name}</h4>
			<h5>가입기간 : ${item.d_min_date} ~ ${item.d_max_date } 년</h5>
			<h5>가입한도 : ${item.d_min_price} ~ ${item.d_max_price } 원</h5>
			<h5>상환방식 : ${item.d_repay} </h5>
			<br><h3 style="color:#008485">금리 : <strong> ${item.d_interest_rate}%</strong></h3> 
		   	<div class="float-right" style="padding-bottom: 1em">
		   		<button type="button" class="btn btn-primary bi bi-cart-dash-fill" onclick="window.location='${path}/loan_apply.do?d_name=${item.d_name}'"></button>
		    </div>	
		 </li>    
		</c:forEach> 			
		</ul>
      ...
      <div>
        ...
        <c:if test="${paging.startPage > 10}">
           <a href="${path}/savings_pro_list.do?pageNum=${paging.prev}" style="color: black">[이전]</a>
        </c:if> 
        
        <!-- 페이지번호 처리 -->
        <c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
           <a href="${path}/savings_pro_list.do?pageNum=${num}"  style="color: black">${num}</a>
        </c:forEach>
        
        <!-- 다음 버튼 활성화 여부 -->
        <c:if test="${paging.endPage < paging.pageCount}">
           <a href="${path}/savings_pro_list.do?pageNum=${paging.next}"  style="color: black">[다음]</a>
        </c:if>
    </div>
    <!-- end Page Content -->
    </div>
