...
for (let i = 0; i < dropList.length; i++) {
    for(let currency_code in country_list){
        ...
        let selected = i == 0 ? currency_code == "KRW" ? "selected" : "" : currency_code == "USD" ? "selected" : "";
...
function getExchangeRate(){
    ...
    exchangeRateTxt.innerText = "계산 중 입니다...";
    let url = `https://v6.exchangerate-api.com/v6/dc6d950f881a90bf8847ea32/latest/${fromCurrency.value}`;
    ...
    fetch(url).then(response => response.json()).then(result =>{
        ...
        exchangeRateTxt.innerText = "뭔가 잘못 됐군요.";
    });
}